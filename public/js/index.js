...
const user_id = '65486431' // tg.initDataUnsafe.user.id
...
$('.screen-sel-sel-block-sel-transfe-st').on('click', (e) => {
    ...
    if (SHID == 'sel') {
        $('.st-transfe').css('border-bottom', '4px solid transparent')
        $('.st-sel').css('border-bottom', '4px solid #f07e2d')
        $(`.sssb-transfe`).css('display', 'block')
        $(`.sssb-sel`).css('display', 'none')

    } else if (SHID == 'transfe') {
        $('.st-sel').css('border-bottom', '4px solid transparent')
        $('.st-transfe').css('border-bottom', '4px solid #f07e2d')
...
const screen_load = (response) => {
    ...
    if (response.data.user.ToRefCode != null) {
...
const topclick = (response) => {
    ...
    for (let i = 0; i < response.data.length; i++) {
        const element = response.data[i];
        mouse.push(`${element.id}, ${parseFloat(element.mouse).toFixed(10)}`)
    }
    mouse.sort((a, b) => {
        return parseFloat(b.split(',')[1]) - parseFloat(a.split(',')[1]);
    });
    for (let cl = 0; cl < mouse.length; cl++) {
        const ms = mouse[cl].split(',')[0];
...
const topcoin = (response) => {
    ...
    for (let i = 0; i < response.data.length; i++) {
        const element = response.data[i];
        coin.push(`${element.id}, ${parseFloat(element.coin).toFixed(10)}`)
    }
    coin.sort((a, b) => {
        return parseFloat(b.split(',')[1]) - parseFloat(a.split(',')[1]);
    });
    for (let c = 0; c < coin.length; c++) {
        const coinArr = coin[c].split(',')[0];
        for (let uc = 0; uc < response.data.length; uc++) {
            const userCoin = response.data[uc];
            if (coinArr == (userCoin.id)) {
                var id = c + 1
...
const RefCode = (code) => {
    axios.post(`${ApiEndpointsUser}/user-ref`, {
        ...
        } else if (response.data == 'нельзя использовать свой код') {
            $('#ref-code-user').css('background', 'red')
        } else {
            $('#ref-code-user').prop('disabled', true);
            $('#ref-code-user').css('background', 'transparent');
            $('#save-code').css('color', '#00ff00');
            $('#save-code').text('Код сохранен');
        }
    })
...
const OfflineClicker = (sec) => {
    axios.post(`${ApiEndpointsUser}/user-new-click-offline`, {
        id_user: user_id,
        sec: sec
    }).then((res) => {
        if (res.data == 'ok') {
