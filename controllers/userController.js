...
class UserController {
    ...
    // async ClickCoin(req, res) { // кликер офлайн
    //     const { id_user } = req.body
    //     const user = await User.findOne({ where: { id_user } })
    //     const newCoin = parseFloat(user.coin) + parseFloat(user.mouse)
    //     user.coin = newCoin
    ...
    async UserSell(req, res) { // продажа монет
        ...
        if (sum == '' || curs == '') {
            ...
            const user = await User.findOne({ where: { id_user } })
            const trns = await Sale.create({ userIdMe: id_user, userNameMe: user.userName, sum, curs })
    ...
    async GameOverOne(req, res) {
        try {
            ...
            console.log(gamer_win.name);
            console.log(summaCoin);
